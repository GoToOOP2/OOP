...
class PasswordEncryptorAdapterTest {
    ...
    fun `encrypt 호출 시 BCrypt 해시를 반환한다`() {
        // when
        val encoded = sut.encrypt(RawPasswordVO.from("password123"))
    ...
    fun `matches - 원문과 해시가 일치하면 true를 반환한다`() {
        ...
        // when & then
        assertThat(sut.matches(RawPasswordVO.from("password123"), encoded)).isTrue()
    }
    ...
    fun `matches - 원문과 해시가 일치하지 않으면 false를 반환한다`() {
        ...
        // when & then
        assertThat(sut.matches(RawPasswordVO.from("wrongpassword"), encoded)).isFalse()
    }
}
